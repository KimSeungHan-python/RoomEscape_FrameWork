...
void ADungeonGenerator::SpawnNextRoom()// 방번호랑 (방 전체 화살표 중에 어디로 생길지 랜덤으로 정함 그리고 스포할 수 없으면)
{
	...
	if (RoomsSpawnedInCurrentPhase >= RoomsPerPhase && CurrentSpecialRoomIdx < SpecialRoomsToBeSpawned.Num() && RoomAmount != 1)
	{
		UE_LOG(LogTemp, Warning, TEXT("SpecialRoomSelected"));
	...
	else if (RoomAmount == 1)//방이 하나 남앗을때 
	{
		LatestSpawnedRoom = this->GetWorld()->SpawnActor<ABossRoom>(BossRoomToBeSpawned);
	...
	if (bCanSpawn)//이걸 통과해야지 실제로 맵에 생성이 됨
	{
		if (bIsSpecialRoom)
		{
			//UE_LOG(LogTemp, Warning, TEXT("SpecialRoomSpawn"));
